...
export default function DashboardClient({ initialKpiData }: { initialKpiData?: { kpis: any[]; source: string } | null }) {
  ...
  const handleQuery = useCallback(async (query: string) => {
    ...
    try {
      ...
      while (true) {
        ...
        for (const eventBlock of events) {
          ...
          try {
            ...
            else if (eventName === 'narasi') { liveNarasiRef.current = payload.text ?? ''; setLiveNarasi(liveNarasiRef.current); }
            else if (eventName === 'result') finalResult = payload as HybridResponse;
      ...
      if (finalResult) setAiResponse(finalResult);
      else if (liveNarasiRef.current) setAiResponse({ narasi: liveNarasiRef.current, visualisasi: { tipe: 'none', konfigurasi: {} }, rekomendasi: [], dataSource: 'SAPA Aceh Tengah', timestamp: new Date().toISOString() });
      else throw new Error('AI tidak mengembalikan respons');
      setStatusText(null);
      liveNarasiRef.current = '';
      setLiveNarasi('');
    } catch (err: any) {
      ...
      const errMsg = err?.name === 'AbortError' ? 'AI membutuhkan waktu terlalu lama (65 detik). Coba pertanyaan yang lebih singkat.' : `Terjadi kesalahan: ${err?.message ?? 'Unknown'}`;
      setError(errMsg);
      setMode('ai-response');
      setAiResponse(null);
      liveNarasiRef.current = '';
    ...
    <div className="space-y-5">
      ...
      {mode === 'ai-response' && !isLoading && error && (
        <div className="bg-[var(--surface-card)] border border-[#C6C3B4] rounded-2xl p-8 text-center">
          ...
          <button onClick={handleReset} className="px-4 py-2 bg-[#1B4332] text-[var(--on-brand)] text-sm rounded-lg hover:bg-[#2D6A4F]">Kembali ke Beranda</button>
        </div>
      )}
          ...
          {liveNarasi ? (
            <div className="mt-4 max-w-2xl mx-auto text-left">
              <div className="bg-[var(--surface-card)] border border-[#C6C3B4] rounded-xl p-4">
                <p className="text-sm text-[#4B5249] leading-relaxed whitespace-pre-wrap">{liveNarasi}<span className="inline-block w-2 h-4 bg-[#1B4332] ml-0.5 animate-pulse" /></p>
              </div>
            </div>
